...
class ProductSearchRoute extends AbstractProductSearchRoute
{
    ...
    private function addSortingFromRequest(Request $request, Criteria $criteria) : void
    {
        ...
        if ($sortkey != '') {
            ...
            $sortings = array();
            $sn = new ProductSortingEntity ('score', 'filter.sortByScore', [ "_score"=> "desc" ] );
            $sortings[]=$sn;
            $sn = new ProductSortingEntity ($params['fsname'], 'filter.'.$params['fsname'], [$params['fsname']=>'desc']);
            $sn->setActive(true);
            $sortings[]=$sn;
    ...
    private function convertToProductSortingEntity(ProductSortingEntity $sorting): ProductSortingEntity
    {
    ...
    private function setSortorderToResult(ProductListingResult $result) : void
    {
        ...
        foreach ($semknoxData['data']['sortData'] as $sd) {
            $sn = new ProductSortingEntity ($sd['shopKey'], $sd['snippet'], $sd['fields']);
            if ($sd['selected']) {
                $sn->setActive(true);
                $selected = $sd['shopKey'];
            }
